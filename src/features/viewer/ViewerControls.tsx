...
export function ViewerControls({ view, onFullscreen }: ViewerControlsProps) {
  ...
  return (
    <div className="viewer-controls">
      <button className="icon-button" type="button" onClick={() => zoomOut(view)} aria-label="Zoom -">
        -
      </button>
      <span className="min-w-12 text-center text-xs font-semibold text-slate-500">
        {Math.round(zoom * 100)}%
      </span>
      <button className="icon-button" type="button" onClick={() => zoomIn(view)} aria-label="Zoom +">
        +
      </button>
      <button className="control-button" type="button" onClick={() => reset(view)}>
        Reset
      </button>
